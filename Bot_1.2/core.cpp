...
// I can imagine this helping the bot performance :)
int PlayerCore::evaluate(board &bd){
    ...
    if(check_win(white)) return inf;
    if(check_win(black)) return -inf;
...
u64 PlayerCore::find_move(board bd){
    ...
    int true_ev = -inf, best = -inf-1;
    u64 best_move = 0, true_best_move = 0;

    int max_depth_reached = 0;

    for(int deep=1; deep<=140-move_count; deep++){
        if(best >= inf)
            break;
        best = -inf-1;
