...
class Rompe:
    ...
    def __putRandomPiece(self, board):
        ...
        return board
    def inicioB(self,jugadas,tablero):

        if jugadas==0:
            tablero[1][3]=1 * self.piecesColor
        if jugadas==1:
            tablero[1][0]=4 * self.piecesColor
        if jugadas==2:
            tablero[1][2]=3 * self.piecesColor
        if jugadas==3:
            tablero[1][1]=2 * self.piecesColor
        if jugadas==4:
            tablero[1][1]=2 * self.piecesColor
            tablero[2][3]=0
        return tablero 
    ...
    def play(self, board):
        ...
        for _ in range(1000):
            if self.currentTurn < 3:
                newBoard = self.inicioB(self.currentTurn,board)
